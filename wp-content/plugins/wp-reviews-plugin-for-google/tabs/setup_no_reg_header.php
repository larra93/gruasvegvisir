...
function trustindex_connect_page($page_details = null, $default_settings = true)
{
...
function trustindex_disconnect_page($settings_delete = true)
{
global $trustindex_pm_google;
global $wpdb;
delete_option( $trustindex_pm_google->get_option_name('page-details') );
...
function trustindex_change_step($step = 5)
{
global $trustindex_pm_google;
if($step < 5)
{
...
trustindex_connect_page($page_details);
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=setup_no_reg');
exit;
}
...
trustindex_disconnect_page(false);
trustindex_connect_page($page_details, false);
